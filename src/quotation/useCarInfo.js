...
import { getCarImages, getCarsData, getCarInfoDetail } from "@/service/home"

function useUpdateEffect(effect, deps) {
  // useRef 用于存储一个 boolean 值，以确定是否为组件的第一次渲染
  const isFirstRender = useRef(true);

  useEffect(() => {
    // 如果是组件的首次渲染，则不要运行effect函数
...
export const useCarImages = (data) => {
    ...
    useEffect(()=> {
        ...
        (async () => {
            console.log(data, state, isLoading.current);
            if(!data || state || isLoading.current) return;
...
export const useCarData = (data) => {
    const [state, setState] = useState()
    
    useEffect(()=> {
        (async () => {
            if(!data || state) return;
            const result = await getCarsData({car_id: data.car_id});
            setState(result)
            console.log('car data', result);
        })();
